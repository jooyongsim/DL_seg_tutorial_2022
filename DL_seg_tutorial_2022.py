...
def evaluate_submission(model, data_loader, device, num_classes, save_eval_path):
    model.eval()
    confmat = utils.ConfusionMatrix(num_classes)
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = "Test:"
    with torch.inference_mode():
        for image, target in metric_logger.log_every(data_loader, 100, header):
            ...
            output = model(image)
            output = output["out"]
            outputs = output.argmax(1).flatten()
            for cnt, output in enumerate(outputs):
                torchvision.utils.save_image(output, os.path.join(save_eval_path, '{cnt}.jpeg')) 
        confmat.reduce_from_all_processes()
    return confmat
...
epochs = 50
lr =0.02
save_eval_path = 'eval_imgs'
saved_ckpt_path = '../segmentation/deeplabv3_resnet50_e29.pth'
prep_submit = True
